...
export default function PaymentListPage() {
  ...
  const calculateDues = () => {
    ...
    return students.map(student => {
      ...
      const totalPaid = payments.reduce((sum, p) => {
        ...
        const pStudent = p.__raw ? p.__raw.student : (p.student || p.Student)
  ...
  const handleDownload = (payment) => {
    ...
    import('jspdf').then(({ default: jsPDF }) => {
      ...
      doc.text('PG/HOSTEL MANAGEMENT', 105, 18, { align: 'center' })
      doc.setFontSize(16)
      ...
      doc.text(`Receipt ID: ${payment.PaymentID}`, 20, 55)
      doc.text(`Generated: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`, 20, 62)
      ...
      doc.setFont('helvetica', 'normal')
      doc.text(payment.Student, 75, yPos)
      ...
      // Save the PDF
      doc.save(`Payment_Receipt_${payment.PaymentID}_${payment.Student.replace(/\s+/g, '_')}.pdf`)
    }).catch(err => {
