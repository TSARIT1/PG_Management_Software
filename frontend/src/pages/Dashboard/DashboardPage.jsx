...
export default function DashboardPage() {
  ...
  const statsRow1 = [
    { icon: FiUsers, value: loading ? '...' : stats.totalStudents.toString(), label: 'Total Students', color: 'blue' },
    { icon: FiTrendingUp, value: loading ? '...' : stats.totalStaff.toString(), label: 'Total Staff', color: 'green' },
    { icon: FiGrid, value: loading ? '...' : stats.totalRooms.toString(), label: 'Total Rooms', color: 'teal' },
    { icon: FiCheckCircle, value: loading ? '...' : stats.occupiedRooms.toString(), label: 'Occupied Rooms', color: 'orange' },
  ]

  const statsRow2 = [
    { icon: FiAlertCircle, value: loading ? '...' : stats.availableRooms.toString(), label: 'Available Rooms', color: 'green' },
    { icon: FiDollarSign, value: loading ? '...' : `₹${stats.totalRevenue.toLocaleString()}`, label: 'Total Revenue', color: 'blue' },
    { icon: FiClock, value: loading ? '...' : `₹${stats.thisMonthRevenue.toLocaleString()}`, label: "This Month's Revenue", color: 'blue' },
    { icon: FiGrid, value: loading ? '...' : stats.fullRooms.toString(), label: 'Full Rooms', color: 'orange' },
  ]
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="page-subtitle" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>Welcome to your PG/Hostel management dashboard</p>
      </motion.div>
      ...
      >
        <div style={{ position: 'relative' }}>
          ...
          <input
            type="text"
            placeholder="Search candidates, rooms, or staff..."
            value={searchQuery}
